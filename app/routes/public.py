...
def globals():
    return {
        ...
        'hero_video_url': current_app.config['HERO_VIDEO_URL'],
    ...
    return jsonify([
        {
            ...
            'image': (
                url_for('static', filename=p.image)
                if p.image
                else None
            ),
            'detail_url': url_for(
...
@public_bp.get('/wholesale')
def wholesale():
    return render_template('public/wholesale.html')


@public_bp.get('/export')
def export():
    products = (
        Product.query
